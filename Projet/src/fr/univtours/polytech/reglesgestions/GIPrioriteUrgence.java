...
public class GIPrioriteUrgence implements GestionInfirmiers {
	...
	public Tuple<Salle, Patient> solution() {
		if (simulation.getListes().getListesAttente().get(ListesAttentes.typeListes.LAIU).size() > 0) {
			...
			Tuple<Salle, Patient> retour =  simulation.getListes().getListesAttente().get(ListesAttentes.typeListes.LAIU).get(0);
		...
		if (simulation.getListes().getListesAttente().get(ListesAttentes.typeListes.LAI).size() > 0) {
			Tuple<Salle, Patient> retour =  simulation.getListes().getListesAttente().get(ListesAttentes.typeListes.LAI).get(0);
	...
	public void ajoutSalle(Salle salle, Patient patient) {
		if (patient.estUrgent()) {
			simulation.getListes().getListesAttente().get(ListesAttentes.typeListes.LAIU).add(new Tuple<Salle, Patient>(salle, patient));	
		}
		simulation.getListes().getListesAttente().get(ListesAttentes.typeListes.LAI).add(new Tuple<Salle, Patient>(salle, patient));	
	}
}
